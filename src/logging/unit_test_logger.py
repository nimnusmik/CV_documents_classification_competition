# src/logging/unit_test_logger.py
"""
...
class UnitTestLogger:
    ...
    def __init__(self, test_name: str, base_log_dir: str = "notebooks"):
        ...
        """
        self.test_name = test_name                                  # 테스트 이름 저장
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")   # 현재 시간 타임스탬프 생성
        
        # 로그 디렉토리 구조 생성
        self.base_dir = Path(base_log_dir) / "modular" / "unit_tests" / test_name / self.timestamp  # 기본 디렉터리 경로
        ...
        self.test_results = {}                              # 테스트 결과 저장 딕셔너리
        self.test_start_time = datetime.now()               # 테스트 시작 시간 기록
        
        # 출력 캡처를 위한 설정
        self.original_stdout = sys.stdout                   # 원본 표준 출력 저장
        self.original_stderr = sys.stderr                   # 원본 표준 에러 저장
        
        self.log_info(f"단위 테스트 시작: {test_name}")       # 테스트 시작 로그
    ...
    def _setup_logger(self) -> logging.Logger:
        """로거 초기화"""
        logger = logging.getLogger(f"unit_test_{self.test_name}")           # 테스트별 로거 생성
        ...
        # 파일 핸들러 추가
        log_file = self.log_dir / f"{self.test_name}_{self.timestamp}.log"  # 로그 파일 경로 생성
    ...
    def save_test_result(self, test_section: str, result: dict):
        ...
        self.test_results[test_section] = {
            **result,                                       # 기존 결과 데이터 병합
            ...
            "section": test_section                         # 테스트 섹션명 추가
        }
        ...
        with open(result_file, 'w', encoding='utf-8') as f:  # 결과 파일 열기
            json.dump(self.test_results[test_section], f, indent=2, ensure_ascii=False)  # JSON 형태로 저장
        
        self.log_info(f"테스트 결과 저장: {test_section}")   # 저장 완료 로그
    ...
    # ---------------------- 테스트 완료 및 정리 함수 ---------------------- #
    # 테스트 완료 및 최종 결과 저장 함수 정의
    def finalize_test(self):
        test_end_time = datetime.now()                                          # 테스트 종료 시간 기록
        test_duration = (test_end_time - self.test_start_time).total_seconds()  # 테스트 소요 시간 계산
        
        # 전체 테스트 요약 딕셔너리 생성
        test_summary = {
            "test_name": self.test_name,                    # 테스트명
            "start_time": self.test_start_time.isoformat(), # 시작 시간
            "end_time": test_end_time.isoformat(),          # 종료 시간
            "duration_seconds": test_duration,              # 소요 시간(초)
            "total_sections": len(self.test_results),       # 총 섹션 수
            "results": self.test_results,                   # 테스트 결과들
            "log_directory": str(self.base_dir)             # 로그 디렉터리 경로
        }
        
        # 최종 요약 파일 저장
        summary_file = self.base_dir / "test_summary.json"  # 요약 파일 경로 생성
        
        # 요약 파일 저장
        with open(summary_file, 'w', encoding='utf-8') as f:  # 요약 파일 열기
            json.dump(test_summary, f, indent=2, ensure_ascii=False)  # JSON 형태로 저장
        
        self.log_success(f"테스트 완료! 총 소요 시간: {test_duration:.2f}초")  # 완료 로그
        self.log_success(f"결과 요약: {summary_file}")       # 요약 파일 경로 로그
        
        # 최종 요약을 콘솔에 출력
        print("\n" + "="*50)                                # 구분선 출력
        print(f"🏁 단위 테스트 완료: {self.test_name}")       # 테스트 완료 메시지
        print("="*50)                                       # 구분선 출력
        print(f"📁 결과 디렉토리: {self.base_dir}")          # 결과 디렉터리 출력
        print(f"⏱️ 소요 시간: {test_duration:.2f}초")        # 소요 시간 출력
        print(f"📊 테스트 섹션 수: {len(self.test_results)}") # 섹션 수 출력
        print("="*50)                                       # 구분선 출력
...
def create_test_logger(test_name: str) -> UnitTestLogger:
    # UnitTestLogger 인스턴스 반환
    return UnitTestLogger(test_name)


# 테스트 섹션 시작 로깅 데코레이터 정의
...
if __name__ == "__main__":
    # 테스트 예제
    logger = create_test_logger("example_test")             # 예제 테스트 로거 생성
    
    # 기본 로깅 테스트
    logger.log_info("테스트 로거 예제 시작")                  # 테스트 시작 로그
    ...
    # 테스트 결과 저장
    logger.save_test_result("example_section", {            # 테스트 결과 저장
        "status": "success",                                # 상태
        "score": 0.95,                                      # 점수
        "details": "예제 테스트 완료"                        # 상세 내용
    })
