...
# 최적화 로그 확인
tail -f logs/optimization/optuna_YYYYMMDD_HHMM.log
```
...
# 캘리브레이션 로그 확인
grep "calibration" logs/pipeline/full_pipeline_*.log
```
...
```bash
# 실시간 로그 확인
tail -f logs/train/train_highperf_advanced_augmentation_*.log
