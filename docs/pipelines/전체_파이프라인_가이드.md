...
#### **파이프라인 실행**
```bash
# 기본 고성능 파이프라인 실행 (설정 파일 필수)
python src/training/train_main.py --config configs/train_highperf.yaml --mode full-pipeline

# 커스텀 설정으로 실행
python src/training/train_main.py --config configs/my_experiment.yaml --mode full-pipeline

# 특정 GPU 지정
CUDA_VISIBLE_DEVICES=0 python src/training/train_main.py --config configs/train_highperf.yaml --mode full-pipeline

# 백그라운드 실행 (긴 학습용)
nohup python src/training/train_main.py --config configs/train_highperf.yaml --mode full-pipeline > training.log 2>&1 &
```
