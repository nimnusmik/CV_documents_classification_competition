...
```mermaid
flowchart TD
    A[📁 모델 로딩<br/>experiments/train/<br/>best_fold*.pth<br/>5개 Fold 모델 로딩] --> B[🖼️ 테스트 데이터<br/>data/raw/test/<br/>data/raw/meta.csv<br/>추론할 이미지 준비]
...
graph TB
    ...
    subgraph "🧠 코어 추론 시스템"
        ...
        UTILS[src/utils/common.py<br/>공통 유틸리티<br/>결과 후처리]
    end
    
    subgraph "🎯 학습된 모델"
        EXP[experiments/train/YYYYMMDD/<br/>model_name/ckpt/<br/>학습 완료 모델]
...
python src/inference/infer_main.py \
    ...
    --ckpt experiments/train/20250908/efficientnet_b3_20250908_0313/ckpt/best_fold0.pth

# 환경변수로 최신 모델 자동 선택
LATEST_MODEL=$(find experiments/train -name "best_fold0.pth" | sort | tail -1)
python src/inference/infer_main.py --config configs/infer.yaml --ckpt $LATEST_MODEL
```
...
python src/inference/infer_main.py \
    ...
    --mode highperf \
    --use-tta \
    --fold-results experiments/train/$(date +%Y%m%d)/*/fold_results
```

### 3. 특정 모델로 TTA 추론
```bash
BEST_MODEL=$(find experiments/train -name "best_fold0.pth" | sort | tail -1)
python src/inference/infer_main.py \
    --config configs/infer.yaml \
    --mode basic \
    --ckpt $BEST_MODEL \
...
```bash
# 사용 가능한 모델 확인
find experiments/train -name "*.pth" | head -10
...
# 모델 파일 크기 확인
ls -lh experiments/train/*/ckpt/*.pth
```
...
# 6. 특정 날짜 모델 사용
TODAY_MODEL=$(find experiments/train/$(date +%Y%m%d) -name "best_fold0.pth" | head -1)
