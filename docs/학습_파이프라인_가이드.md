...
graph TD
    ...
    F --> H[실험 결과 저장<br/>experiments/train/YYYYMMDD/<br/>모델 체크포인트 관리]
    G --> H
...
flowchart LR
    ...
    D --> E[💾 최적 모델 저장<br/>experiments/train/<br/>YYYYMMDD/model/ckpt/<br/>베스트 체크포인트]
...
graph TB
    ...
    subgraph "💾 출력 결과"
        EXP[experiments/train/YYYYMMDD/<br/>실험 결과 저장소]
...
```bash
# 최신 모델 확인
ls -la experiments/train/$(date +%Y%m%d)/*/ckpt/
...
python -c "
import json, glob
metric_files = glob.glob('experiments/train/$(date +%Y%m%d)/*/metrics.json')
...
# 24. 모델 크기 확인
ls -lh experiments/train/$(date +%Y%m%d)/*/ckpt/*.pth
```
